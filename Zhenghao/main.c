...
// value suggest by chatgpt, might need to be adjusted
/* USER CODE END PD */
...
/* USER CODE BEGIN PV */
// global variables
uint16_t buffer[N] = {0}; // buffer for a moving window to scan through the data
uint8_t index = 0; // index for the buffer
uint32_t sum = 0; // sum of the buffer
uint8_t downsample_toggle = 0; // this is not real downsampling, but a toggle to essentially halve the sampling rate
